...
region_income_combo AS (
  SELECT r.Region, i.income_group
  FROM region_list r
  CROSS JOIN income_list i
),
country_counts AS (
...
SELECT
  ric.Region,
  ric.income_group,
  ROUND(COALESCE(cc.cnt, 0) * 100.0 / rt.total, 1) AS pct
FROM region_income_combo ric
LEFT JOIN country_counts cc
  ON ric.Region = cc.Region AND ric.income_group = cc.`Income Group`
JOIN region_totals rt
  ON ric.Region = rt.Region
...
SELECT
  cc.Region,
  ROUND(SUM(CASE WHEN `Income Group` = 'High income' THEN cnt ELSE 0 END) * 100.0 / rt.total, 1) AS "High",
  ROUND(SUM(CASE WHEN `Income Group` = 'Upper middle income' THEN cnt ELSE 0 END) * 100.0 / rt.total, 1) AS "Upper middle",
  ROUND(SUM(CASE WHEN `Income Group` = 'Lower middle income' THEN cnt ELSE 0 END) * 100.0 / rt.total, 1) AS "Lower middle",
  ROUND(SUM(CASE WHEN `Income Group` = 'Low income' THEN cnt ELSE 0 END) * 100.0 / rt.total, 1) AS "Low"
FROM country_counts cc
JOIN region_totals rt ON cc.Region = rt.Region
GROUP BY cc.Region
ORDER BY cc.Region;
""")
