...
SELECT
  Region,
  CAST(SUM(CASE WHEN `Income Group` = 'High income' THEN 1 ELSE 0 END) AS DECIMAL(10,1)) AS "High",
  CAST(SUM(CASE WHEN `Income Group` = 'Upper middle income' THEN 1 ELSE 0 END) AS DECIMAL(10,1)) AS "Upper middle",
  CAST(SUM(CASE WHEN `Income Group` = 'Lower middle income' THEN 1 ELSE 0 END) AS DECIMAL(10,1)) AS "Lower middle",
  CAST(SUM(CASE WHEN `Income Group` = 'Low income' THEN 1 ELSE 0 END) AS DECIMAL(10,1)) AS "Low",
  CAST(SUM(CASE WHEN `Income Group` IS NULL THEN 1 ELSE 0 END) AS DECIMAL(10,1)) AS "Other",
...
WITH Counts AS (
  ...
  FROM
    wdi_country
  GROUP BY Region, COALESCE(`Income Group`, 'Other')
),
Totals AS (
  SELECT Region, SUM(country_count) AS region_total FROM Counts GROUP BY Region
),
IncomeTotals AS (
  SELECT `Income Group`, SUM(country_count) AS income_total FROM Counts GROUP BY `Income Group`
),
GrandTotal AS (
...
SELECT
  ...
  i.income_total,
  g.total_countries,
  ROUND((c.country_count * 100.0 / g.total_countries), 2) AS percent_of_total
FROM Counts c
JOIN Totals t ON c.Region = t.Region
JOIN IncomeTotals i ON c.`Income Group` = i.`Income Group`
...
WITH Counts AS (
  ...
  FROM
    wdi_country
  GROUP BY COALESCE(`Income Group`, 'Other')
),
GrandTotal AS (
  SELECT SUM(country_count) AS total_countries FROM Counts
)
