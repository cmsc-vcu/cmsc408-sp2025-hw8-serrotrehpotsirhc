...
IncomeGroups AS (
  SELECT DISTINCT `Income Group` FROM world_bank_data.wdi_country WHERE `Income Group` IS NOT NULL
),
AllCombinations AS (
  SELECT r.Region, i.`Income Group`
  FROM Regions r
  CROSS JOIN IncomeGroups i
),
CountryCounts AS (
...
Combined AS (
  SELECT ac.Region, ac.`Income Group`, COALESCE(cc.country_count, 0) AS country_count
  FROM AllCombinations ac
  LEFT JOIN CountryCounts cc
    ON ac.Region = cc.Region AND ac.`Income Group` = cc.`Income Group`
),
RegionTotals AS (
...
GrandTotal AS (
  SELECT SUM(country_count) AS total_countries FROM Combined
)
SELECT
  c.Region,
  c.`Income Group`,
  ...
  it.income_total,
  gt.total_countries,
  ROUND((c.country_count * 100.0 / gt.total_countries), 2) AS percent_of_total
FROM
  Combined c
JOIN RegionTotals rt ON c.Region = rt.Region
JOIN IncomeTotals it ON c.`Income Group` = it.`Income Group`
JOIN GrandTotal gt
