...
WHERE `Table Name` != 'Country' OR `Table Name` IS NULL
ORDER BY `Short Name`
""")
```
...
FROM
  world_bank_data.wdi_country
WHERE
  `Income Group` = 'Low income' AND `Table Name` = 'Country'
GROUP BY
  Region,
  `Income Group`
ORDER BY
...
SELECT
  AllPairs.Region,
  AllPairs.`Income Group`
FROM
  AllPairs
LEFT JOIN
  ExistingPairs
  ON AllPairs.Region = ExistingPairs.Region
  AND AllPairs.`Income Group` = ExistingPairs.`Income Group`
WHERE
  ExistingPairs.Region IS NULL
ORDER BY
  AllPairs.Region,
  AllPairs.`Income Group`
""")
